...
class Racetrack:
    ...
    def __init__(
        ...
        self.actions = [
            (i, j) for j in acceleration for i in acceleration
        ...
        self.velocities = np.arange(
            v_min, v_max + 1, 1
        )  # list of all possible velocities
        ...
        self.threshold = (
            0.02
        )  # if the change of Q-value is below the threshold, we can assume that it is stabilized
        ...
        if self.within_track() and self.track[self.y, self.x] != "#":
            if self.v_y == 0:
                if (
                    "#"
                    in self.track[
                        y_temp, min(self.x, x_temp) : max(self.x, x_temp)
                    ].ravel()
                ):
                    self.x = x_temp
                ...
                if (
                    "#"
                    in self.track[
                        min(self.y, y_temp) : max(self.y, y_temp), self.x
                    ].ravel()
                ):
                    self.y = y_temp
                    self.v_y, self.v_x = 0, 0
                ...
                if (
                    "#"
                    in self.track[
                        min(self.y, y_temp) : max(self.y, y_temp),
    ...
    def make_policy(self):
        ...
        for y in range(self.track.shape[0]):
            for x in range(self.track.shape[1]):
                for v_y in self.velocities:
                    for v_x in self.velocities:
                        self.policy[(y, x, v_y, v_x)] = self.actions[
                            np.argmax(self.Q[y, x, v_y, v_x])
                        ]
    ...
    def run(self):
        ...
        self.iterations = {"1": 40, "2": 3000000, "3": 3000000}
